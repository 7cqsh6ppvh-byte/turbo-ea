...
合规扫描范围内的卡片会在其详情页上额外暴露一个**合规**标签页（由 `security_compliance.view` 控制）。它列出当前与该卡片关联的每条发现，并提供与 GRC 视图相同的「确认 / 接受 / **创建风险** / **打开风险**」操作——这样 Application Owner 即可在不离开卡片的情况下对自己的发现进行分诊。同样的自动隐藏规则也适用于卡片详情中的**风险**标签页：只有当卡片实际存在关联条目时，这两个标签页才会出现，因此没有 GRC 活动的卡片不会带着空标签页占用空间。
