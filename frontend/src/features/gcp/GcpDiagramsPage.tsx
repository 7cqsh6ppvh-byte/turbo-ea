...
export default function GcpDiagramsPage() {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
        ...
        <Button variant="contained" sx={{ bgcolor: "#4285F4", "&:hover": { bgcolor: "#2A6DD9" } }} onClick={() => setCreateOpen(true)}>
          {t("newDiagram")}
        </Button>
      </Box>
        ...
        >
          {diagrams.map((d) => (
            <Card key={d.id} variant="outlined">
              <CardActionArea onClick={() => navigate(`/gcp/${d.id}/edit`)}>
                <CardContent>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleCreate} disabled={!newName.trim() || creating}
            sx={{ bgcolor: "#4285F4", "&:hover": { bgcolor: "#2A6DD9" } }}>
