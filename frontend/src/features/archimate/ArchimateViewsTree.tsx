...
export function ArchimateViewsTree({ currentDiagramId }: Props) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCreateOpen(false);
      setNewName("");
      navigate(`/archimate/${created.id}/edit`);
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        <Typography variant="caption" sx={{ fontWeight: 700, fontSize: "10px", color: "text.secondary", textTransform: "uppercase", letterSpacing: 0.5, flex: 1 }}>
          {t("sidebar.diagramsHeader")}
        </Typography>
        <IconButton size="small" onClick={() => setCreateOpen(true)} aria-label={t("sidebar.newDiagram")}>
        ...
        {loading ? (
          ...
          diagrams.map((d) => {
            ...
            return (
              <Box
                key={d.id}
                onClick={() => navigate(`/archimate/${d.id}/edit`)}
                sx={{
                  display: "flex",
                  alignItems: "center",
                  gap: 1,
