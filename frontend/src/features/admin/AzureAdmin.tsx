...
export default function AzureAdmin() {
  const { t } = useTranslation("azure");
  const { azureEnabled, invalidateAzure } = useAzureEnabled();
  ...
  async function loadStatus() {
    try {
      const s = await api.get<AzureStatus>("/settings/azure-status");
      setStatus(s);
    } catch {}
  }
  ...
  async function handleToggle(enabled: boolean) {
    setToggling(true);
    try {
      await api.patch("/settings/azure-enabled", { enabled });
      invalidateAzure(enabled);
      await loadStatus();
    } finally {
      setToggling(false);
    }
  ...
  async function handleRollback() {
    setRolling(true);
    try {
      await api.delete("/settings/azure-enabled");
      invalidateAzure(false);
      setStatus(null);
    } finally {
      setRolling(false);
  ...
  return (
    <Box>
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
        <MaterialSymbol icon="cloud" size={32} color="#0089D6" />
        <Box>
      ...
      <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
        <FormControlLabel
          ...
          label={azureEnabled ? t("admin.deactivate") : t("admin.activate")}
        />
      </Paper>
