...
export default function C4Admin() {
  ...
  const { c4Enabled, invalidateC4 } = useC4Enabled();
  ...
  async function loadStatus() {
    try {
      const s = await api.get<C4Status>("/settings/c4-status");
      setStatus(s);
    } catch {}
  }
  ...
  async function handleToggle(enabled: boolean) {
    setToggling(true);
    try {
      await api.patch("/settings/c4-enabled", { enabled });
      invalidateC4(enabled);
      await loadStatus();
    } finally {
      setToggling(false);
    }
  ...
  return (
    <Box>
      <Stack direction="row" alignItems="center" spacing={2} sx={{ mb: 3 }}>
        <MaterialSymbol icon="account_tree" size={32} color="#1168bd" />
        <Box>
      ...
      <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
        <FormControlLabel
          ...
          label={c4Enabled ? t("admin.deactivate") : t("admin.activate")}
        />
      </Paper>
