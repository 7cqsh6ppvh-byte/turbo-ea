...
export default function AzureDiagramsPage() {
  ...
  useEffect(() => {
    api
      .get("/diagrams?type=azure-diagram")
      .then((resp) => {
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 3 }}>
        ...
        <Button variant="contained" sx={{ bgcolor: "#0089D6", "&:hover": { bgcolor: "#006BAA" } }} onClick={() => setCreateOpen(true)}>
          {t("newDiagram")}
        </Button>
      </Box>
        ...
        >
          {diagrams.map((d) => (
            <Card key={d.id} variant="outlined">
              <CardActionArea onClick={() => navigate(`/azure/${d.id}/edit`)}>
                <CardContent>
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleCreate} disabled={!newName.trim() || creating}
            sx={{ bgcolor: "#0089D6", "&:hover": { bgcolor: "#006BAA" } }}>
