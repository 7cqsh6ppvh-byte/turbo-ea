/**
 * useUmlEnabled — module-level singleton that caches whether the UML
 * feature is enabled. Defaults to false (opt-in).
...
export function invalidateUmlEnabled(v: boolean) {
  _notify(v);
}
...
function _fetch(): Promise<void> {
  ...
  _inflight = (async () => {
    try {
      const res = await api.get<{ enabled: boolean }>("/settings/uml-enabled");
      _notify(res.enabled);
    } catch {
