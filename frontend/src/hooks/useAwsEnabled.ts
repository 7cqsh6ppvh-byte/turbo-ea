...
export function invalidateAwsEnabled(v: boolean) {
  _notify(v);
}
...
function _fetch(): Promise<void> {
  ...
  _inflight = (async () => {
    try {
      const res = await api.get<{ enabled: boolean }>("/settings/aws-enabled");
      _notify(res.enabled);
    } catch {
...
export function useAwsEnabled() {
  ...
  return { awsEnabled: enabled, awsLoaded: loaded, invalidateAws: invalidate };
}
