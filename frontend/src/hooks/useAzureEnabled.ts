...
export function invalidateAzureEnabled(v: boolean) {
  _notify(v);
}
...
function _fetch(): Promise<void> {
  ...
  _inflight = (async () => {
    try {
      const res = await api.get<{ enabled: boolean }>("/settings/azure-enabled");
      _notify(res.enabled);
    } catch {
...
export function useAzureEnabled() {
  ...
  return { azureEnabled: enabled, azureLoaded: loaded, invalidateAzure: invalidate };
}
