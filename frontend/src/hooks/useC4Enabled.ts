/**
 * useC4Enabled — module-level singleton that caches whether the C4 Model
...
export function invalidateC4Enabled(v: boolean) {
  _notify(v);
}
...
function _fetch(): Promise<void> {
  ...
  _inflight = (async () => {
    try {
      const res = await api.get<{ enabled: boolean }>("/settings/c4-enabled");
      _notify(res.enabled);
    } catch {
